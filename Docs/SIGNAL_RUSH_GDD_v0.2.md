...
| **F2** | 점프 | 상수 중력의 이차 궤적 기반 단일 점프 ([§4.1](#41-obj-player--플레이어)) | 실제 물리와 F6 도달 계산이 일치 |
...
**점프 모델 (F2)** — 입력 순간 고정 수직 속도를 부여하고 상승과 낙하에 일정한 중력을 적용한다. 따라서 궤적은 청크 도달 계산에 그대로 쓸 수 있는 이차함수다.

`TUNE-P2`, `TUNE-P3` 감쇠 중력은 구현하지 않는다. 체공감보다 [SYS-SPAWN](#54-sys-spawn--절차적-생성)이 실제 점프와 같은 단순한 식으로 도달 가능성을 계산하는 것을 우선한다.

**공격 (F3 / F4)** — 단일 입력이 두 결과를 만든다.

- 공격 입력 순간 판정이 **장애물** 콜라이더와 겹침 → 파괴, 콤보 +1
- 공격 입력 시점에 **총알**이 씬에 존재 → 히트박스 무관하게 패링 성립, 콤보 +1
- 둘 다 아니면 헛스윙 (패널티 없음, 다만 이동 중 판정 시간 손실)
- 공격 판정은 입력 순간에 끝나고 이후에는 `attackDuration` 동안 캐릭터 연출만 재생한다. 재입력 제한은 입력 시점부터 별도의 `attackCooldown`으로 관리하며 두 시간을 합산하지 않는다.
...
- 즉사 없음. 낙사 시 콤보를 0으로 초기화하고 다음 청크 시작점보다 5유닛 위에서 낙하 리스폰한다. 리스폰 중에는 조작할 수 없고 종료 후 기본 속도로 복귀한다.
...
### 5.1 SYS-PHYSICS — 물리 상호작용

- 플레이어는 중력을 받는 `Dynamic Rigidbody2D`이며, 고정 지형·간판과는 일반 충돌해 진행이 차단된다.
- 밀 수 있는 상자는 플레이어 몸통과 물리 충돌하지 않는다. 플레이어의 전방 트리거가 상자만 이동시켜, 플레이어의 자동 전진 속도는 충돌 반작용으로 변하지 않는다.
- `mass` 증대로 플레이어를 덜 밀리게 하는 방식은 사용하지 않는다. 동적 Rigidbody 간 충돌은 양쪽에 반작용이 적용되므로, “상자만 밀기”는 레이어 분리와 트리거 상호작용으로 구현한다.

### 5.2 SYS-COMBO — 콤보 / 아드레날린 (F5)
...
| TUNE-P2 | 점프 감쇠 시작 `T1` | 구현 안 함 | — | 이차 궤적 유지를 위해 폐기 |
| TUNE-P3 | 점프 감쇠 종료 `T2` | 구현 안 함 | — | 이차 궤적 유지를 위해 폐기 |
| TUNE-P4 | 최대 체력 | 5 | 3–5 | |
...
| Q3 | 낙사 처리 | 결정됨 | 다음 청크 5유닛 위 리스폰, 콤보 초기화 |
...
**코딩 컨벤션** — 클래스/메서드/public 프로퍼티 PascalCase, Unity 직렬화 필드와 지역변수 camelCase, 그 외 private 필드 `_camelCase`, 인터페이스 `IPascalCase`. private 필드는 `[SerializeField] private`로 노출하고 public 필드는 쓰지 않는다. 한 파일에 한 클래스, Unity 이벤트 함수는 클래스 최상단에 순서대로. `Update()`에서 `GetComponent` / `Find` 금지 — 캐싱. 매직 넘버 금지 — §6의 값은 전부 직렬화 필드나 상수로. 리팩터링에서 `[Obsolete]` 멤버와 호출부는 함께 제거한다.
